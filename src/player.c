...
void playerDoMovement(Player* player)
{
    ...
    vx *= speed;
    vy *= speed;
    if (!worldGetCollisionInArea(player->x - 0.1 + vx, player->y - 0.1, 0.2,
                                 0.2)) {
        player->x += vx;
    }
    if (!worldGetCollisionInArea(player->x - 0.1, player->y - 0.1 + vy, 0.2,
                                 0.2)) {
        player->y += vy;
    }
}
...
void playerDoShooting(Player* player)
{
    ...
    if (hit.hit && hit.monster != NULL &&
        hitSqrDist < player->weapon.maxSqrDistance) {
...
void playerSelectWeapon(Player* player, WeaponType type)
{
    ...
    for (size_t i = 0; i < weapondefsCount; i++) {
        ...
        if (weapondef.weaponType == type) {
            ...
            player->weapon.maxSqrDistance =
                weapondef.maxDistance * weapondef.maxDistance;

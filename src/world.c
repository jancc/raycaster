...
void worldSetTile(uint32_t x, uint32_t y, uint8_t value)
{
    ...
    if (value != 0)
        worldSetCollision(x, y,
                          TILE_COLLIDER_EAST | TILE_COLLIDER_NORTH |
                              TILE_COLLIDER_SOUTH | TILE_COLLIDER_WEST);
    else
        worldSetCollision(x, y, 0);
}
...
uint8_t worldGetCollision(uint32_t x, uint32_t y)
{
    if (x > tilemapWidth || y > tilemapHeight)
        return TILE_COLLIDER_EAST | TILE_COLLIDER_NORTH | TILE_COLLIDER_SOUTH |
               TILE_COLLIDER_WEST;
    return tilemapCollisions[y * tilemapHeight + x];
}

uint8_t worldGetCollisionInArea(double areaX, double areaY, double areaW,
                                double areaH)
{
...
Player* worldGetPlayer() { return player; }
...
Monster** worldGetMonsters() { return monsters; }

size_t worldGetMonstersSize() { return monstersArraySize; }
    ...
    for (size_t i = 0; i < monstersArraySize; i++) {
        if (monsters[i] != NULL) {
            ...
            if (testRayBoxIntersection(ray, &aabb)) {
                ...
                double sqrDistance =
                    vec2SqrDist(ray->x, ray->y, monsters[i]->x, monsters[i]->y);
...
uint8_t worldHitscan(Ray* ray, HitscanOut* out, uint8_t tiles, uint8_t monsters)
{
    ...
    if (monster != NULL) {
        double sqrMonsterDist = (monster->x - ray->x) * (monster->x - ray->x) +
                                (monster->y - ray->y) * (monster->y - ray->y);
        double sqrTileDist =
            (x - ray->x) * (x - ray->x) + (y - ray->y) * (y - ray->y);
